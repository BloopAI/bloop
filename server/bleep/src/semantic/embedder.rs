...
pub use gpu::ggml::LocalEmbedder;
#[cfg(all(target_os = "windows"))]
pub use gpu::onnx_directml::LocalEmbedder;
...
mod gpu {
    ...
    pub mod onnx_directml {
        ...
        pub struct LocalEmbedder {
            session: Arc<tokio::sync::Mutex<ort::Session>>,
            tokenizer: Tokenizer,
        }
        ...
        impl Embedder for LocalEmbedder {
            ...
            async fn batch_embed(&self, log: Vec<&str>) -> anyhow::Result<Vec<Embedding>> {
                ...
                tracing::info!("acquiring lock");
                let lock = self.session.lock().await;
                tracing::info!("lock acquired {:?}", l_start.elapsed());

                let allocator = lock.allocator();

                let mut embeddings = vec![];
                for sequence in log.into_iter() {
                    let tokenizer_output = self.tokenizer.encode(sequence, true).unwrap();
                    ...
                    let sequence_embedding = &*output_tensor.view();
                    let pooled = sequence_embedding.mean_axis(Axis(1)).unwrap();
                    embeddings.push(pooled.to_owned().as_slice().unwrap().to_vec());
                }
                Ok(embeddings)
            }
        }
