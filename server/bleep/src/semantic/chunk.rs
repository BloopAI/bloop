...
pub fn by_tokens<'s>(
    ...
    while let Some(index_diff) = starts[s..].iter().position(|&p| p - offset > max_tokens) {
        ...
        if next_s == s {
            ...
            loop {
                ...
                if end > end_offset {
                    add_token_range(&mut chunks, src, offsets, offset, end_offset, &mut last_line, &mut last_byte);
                    break;
                }
                add_token_range(&mut chunks, src, offsets, offset, end, &mut last_line, &mut last_byte);
    ...
    if offset < offsets.len() {
        add_token_range(&mut chunks, src, offsets, offset, offsets.len(), &mut last_line, &mut last_byte);
    }
    chunks
}
