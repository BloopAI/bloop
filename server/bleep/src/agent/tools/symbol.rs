...
impl Agent {
    ...
    pub async fn symbol_search(&mut self, symbol: &String, path_idx: &usize) -> Result<String> {
        let path =str::to_owned(self.paths().nth(path_idx.clone()).expect("invalid path alias")); 
        ...
        let content = self.get_file_content(&path.clone()).await?.expect("file not found");
        dbg!("{}", path.clone());
        ...
        let token_response = inner_handle(
            token_request,
