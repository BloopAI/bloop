...
import NewResultsPage from './ResultsPage';
...
const ResultsPage = () => {
  ...
  const { searchQuery, data, loading } = useSearch<
    GeneralSearchResponse | DirectorySearchResponse | FileSearchResponse
  >();
  ...
  const [renderPage, setRenderPage] = useState<'results' | 'repo'>();

  useEffect(() => {
    if (!data?.data[0]) {
      return;
    }
    switch (data?.data[0].kind) {
      case 'dir':
        setRenderPage('repo');
        break;
      default:
  ...
  return (
    <PageTemplate>
      {loading && !data ? <Skeleton /> : newRender()}
